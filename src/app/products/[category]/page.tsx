...
import { ProductDetailActions } from "./[slug]/ProductDetailActions";
import {
  ShieldCheck,
  Sprout,
  Package,
...
export function generateStaticParams() {
  const categoryParams = PRODUCT_CATEGORIES.map((cat) => ({
    category: cat.slug,
  }));
...
export function generateMetadata({ params }: PageProps): Metadata {
  const categoryData = PRODUCT_CATEGORIES.find((c) => c.slug === params.category);
  if (categoryData) {
    return {
      title: `${categoryData.name} Portfolio | Shimanzu Chemicals`,
  ...
  return { title: "Products | Shimanzu Chemicals" };
}

export default function CategoryOrProductPage({ params }: PageProps) {
  const categoryData = PRODUCT_CATEGORIES.find((c) => c.slug === params.category);
  ...
  if (categoryData) {
    return (
      <div className="space-y-12 sm:space-y-16 pb-20">
        ...
        {/* Product Discovery with Pre-selected Category */}
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ProductDirectory initialCategory={categoryData.id} />
        </section>
      </div>
    );
  }

  // If it's a direct product slug page (e.g. /products/fluza, /products/arobic, /products/regal)
  const product = getProductBySlug(params.category);
  if (product) {
